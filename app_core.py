import streamlit as st
from pathlib import Path
import base64

# -------------------------------------------------------
# MUST BE FIRST STREAMLIT COMMAND
# -------------------------------------------------------
st.set_page_config(
    page_title="CYBERPUNK QUOTES",
    page_icon="images/cyberpunk.ico",
    layout="wide",
    initial_sidebar_state="expanded",
)

# -------------------------------------------------------
# CONSOLIDATED SIDEBAR / Z-INDEX / VIDEO / DARK MODE FIX
# -------------------------------------------------------
st.markdown("""
<style>

[data-testid="stToolbar"],
[data-testid="stDecoration"],
header, footer {
    display: none !important;
    visibility: hidden !important;
}

/* Sidebar stays above video */
[data-testid="stSidebar"] {
    position: fixed !important;
    left: 0 !important;
    top: 0 !important;
    width: 260px !important;
    height: 100vh !important;
    z-index: 100000 !important;
    transform: none !important;
    background: rgba(13,13,13,0.95) !important;
    box-shadow: 0 8px 30px rgba(0,0,0,0.6) !important;
}

/* Desktop — push content right */
@media (min-width: 700px) {
    [data-testid="stAppViewContainer"] {
        margin-left: 260px !important;
        width: calc(100% - 260px) !important;
        min-width: 0 !important;
        background: transparent !important;
    }
    .block-container {
        max-width: calc(100% - 260px) !important;
        padding-left: 1.5rem !important;
        padding-right: 1.5rem !important;
    }
}

/* Mobile — overlay sidebar */
@media (max-width: 699px) {
    [data-testid="stAppViewContainer"] {
        margin-left: 0 !important;
        width: 100% !important;
        min-width: 0 !important;
        background: transparent !important;
    }
    .block-container {
        max-width: 100% !important;
        padding-left: 1rem !important;
        padding-right: 1rem !important;
    }
}

/* Keep content above video */
[data-testid="stAppViewContainer"],
[data-testid="stMain"],
.block-container {
    position: relative !important;
    z-index: 50000 !important;
}

/* Keep everything transparent so video background shows */
html, body,
[data-testid="stAppViewContainer"],
[data-testid="stApp"],
[data-testid="stBody"],
[data-testid="stMain"],
.block-container {
    background: transparent !important;
}

/* Dark mode overlay */
[data-testid="stAppViewContainer"] {
    background-color: rgba(10,10,10,0.35) !important;
    color: #e0e0e0 !important;
}
section[data-testid="stSidebar"] {
    background-color: rgba(13,13,13,0.95) !important;
    color: #e0e0e0 !important;
}

/* Responsive charts */
.stPlotlyChart, .plotly, .stPlotlyChart > div {
    max-width: 100% !important;
    height: auto !important;
}

/* Video background */
video[playsinline], video {
    position: fixed !important;
    top: 0 !important;
    left: 0 !important;
    width: 100vw !important;
    height: 100vh !important;
    object-fit: cover !important;
    z-index: -5 !important;
}

body, #root, #MainMenu, [data-testid="stApp"] {
    overflow: auto !important;
}

</style>
""", unsafe_allow_html=True)

# Black hydration background (no white flash)
st.markdown("""
<style>
html, body {
    background: #000 !important;
    height: 100% !important;
    min-height: 100% !important;
}
</style>
""", unsafe_allow_html=True)

# -------------------------------------------------------
# MAIN APP
# -------------------------------------------------------
def run_app():

    import streamlit.components.v1 as components
    import yfinance as yf
    import requests
    from io import BytesIO
    from PIL import Image
    import datetime
    import time
    import pandas as pd
    import plotly.graph_objects as go

    from app_render import (
        render_company_header,
        render_matplotlib_cyberpunk_chart,
        render_plotly_fallback
    )

    def safe_markdown(html: str):
        st.markdown(html, unsafe_allow_html=True)

    # Transparent base styling
    safe_markdown("""
    <style>
    html, body, [data-testid="stBody"], [data-testid="stApp"],
    [data-testid="stAppViewContainer"], [data-testid="stMain"],
    section.main, .block-container {
        background: transparent !important;
    }
    </style>
    """)

    safe_markdown("""
    <style>
    [data-testid="stMetricLabel"],
    [data-testid="stMetricValue"] {
        color: #00eaff !important;
        text-shadow: 0 0 6px rgba(0, 234, 255, 0.8);
    }
    </style>
    """)

    # Try to load custom Cyberpunk CSS
    css_path = Path("cyberpunk_style_embedded.css")
    if css_path.exists():
        try:
            safe_markdown(f"<style>{css_path.read_text()}</style>")
        except:
            pass
    else:
        safe_markdown("""
        <style>
        @import url('https://fonts.googleapis.com/css2?family=Major+Mono+Display&display=swap');
        .cyberpunk-title {
            font-family: 'Major Mono Display', monospace;
            font-size: 58px;
            color: #00eaff;
            text-align: center;
            letter-spacing: 2px;
            text-shadow: 0 0 8px rgba(0,234,255,0.9),
                         0 0 18px rgba(0,128,170,0.25);
            position: relative;
            top: -25px;
            margin-bottom: 6px;
        }
        </style>
        """)

    # -------------------------------------------------------
    # VIDEO BACKGROUND (local or remote)
    # -------------------------------------------------------
    def try_local_video(path: Path):
        try:
            if path.exists():
                data = path.read_bytes()
                b64 = base64.b64encode(data).decode()
                safe_markdown(f"""
                <video autoplay muted loop playsinline>
                    <source src="data:video/mp4;base64,{b64}" type="video/mp4">
                </video>
                """)
                return True
        except:
            pass
        return False

    video_ok = try_local_video(Path("videos/cyberpunk_light.mp4"))

    if not video_ok:
        components.html("""
        <video autoplay muted loop playsinline>
            <source src="https://github.com/eviltosh/final_cyberpunk_quotes_redux_V4/releases/download/v1.0/cyberpunk_light.mp4" type="video/mp4">
        </video>
        """, height=0, width=0)

    # -------------------------------------------------------
    # SIDEBAR CONTROLS
    # -------------------------------------------------------
    st.sidebar.header("⚙️ Controls")

    tickers_input = st.sidebar.text_input("Enter stock tickers:", "AAPL, TSLA, NVDA")
    period = st.sidebar.selectbox("Time Range", ["1mo", "3mo", "6mo", "1y", "2y", "5y", "max"])
    refresh_rate = st.sidebar.slider("Auto Refresh (seconds)", 10, 300, 60)

    st.sidebar.subheader("🔑 API Keys")
    finnhub_api = st.sidebar.text_input("Finnhub API Key", value="", type="password")

    st.sidebar.subheader("🌅 Chart Background")
    bg_choice = st.sidebar.selectbox("Background:", ["Beach 1", "Beach 2", "Classic", "Upload Your Own"])

    uploaded_bg = None
    if bg_choice == "Upload Your Own":
        uploaded_bg = st.sidebar.file_uploader("Upload image", type=["jpg", "jpeg", "png"])

    bg_image = None
    if uploaded_bg:
        from PIL import Image as PILImage
        try:
            bg_image = PILImage.open(uploaded_bg)
        except:
            bg_image = None
    else:
        try:
            if bg_choice == "Beach 1":
                bg_image = Image.open("images/1.jpg")
            elif bg_choice == "Beach 2":
                bg_image = Image.open("images/2.jpg")
        except:
            bg_image = None

    # -------------------------------------------------------
    # TITLE
    # -------------------------------------------------------
    safe_markdown("""
    <br>
    <div style="text-align:center;">
        <h1 class="cyberpunk-title">CYBERPUNK QUOTES</h1>
        <br><br>
    </div>
    """)

    tickers = [t.strip().upper() for t in tickers_input.split(",") if t.strip()]

    if "last_refresh" not in st.session_state:
        st.session_state["last_refresh"] = time.time()
    else:
        if time.time() - st.session_state["last_refresh"] > refresh_rate:
            st.session_state["last_refresh"] = time.time()
            st.rerun()

    @st.cache_data(ttl=3600)
    def get_stock_data(ticker, period):
        try:
            return yf.Ticker(ticker).history(period=period)
        except:
            return pd.DataFrame()

    @st.cache_data(ttl=3600)
    def get_info_cached(ticker):
        try:
            return yf.Ticker(ticker).get_info()
        except:
            return {}

    @st.cache_data(ttl=1800)
    def get_company_news(symbol, key):
        if not key:
            return []
        FINNHUB = "https://finnhub.io/api/v1/company-news"
        today = datetime.date.today()
        past = today - datetime.timedelta(days=30)
        try:
            r = requests.get(FINNHUB, params={
                "symbol": symbol,
                "from": past.isoformat(),
                "to": today.isoformat(),
                "token": key
            }, timeout=10)
        except:
            return []
        if r.status_code == 200:
            data = r.json()
            return [n for n in data if n.get("headline")]
        return []

    # -------------------------------------------------------
    # MAIN LOOP
    # -------------------------------------------------------
    for ticker in tickers:

        info = get_info_cached(ticker)
        hist = get_stock_data(ticker, period)

        if hist is None or hist.empty:
            st.warning(f"No data for {ticker}")
            continue

        render_company_header(info, ticker)

        chart_done = render_matplotlib_cyberpunk_chart(hist, ticker, bg_image)
        if not chart_done:
            render_plotly_fallback(hist, ticker)

        # METRICS
        r1c1, r1c2 = st.columns(2)
        r2c1, r2c2 = st.columns(2)

        price = info.get("currentPrice") or info.get("regularMarketPrice")
        cap = info.get("marketCap")
        high = info.get("fiftyTwoWeekHigh")
        low = info.get("fiftyTwoWeekLow")

        r1c1.metric("Current Price", f"${price:,.2f}" if price else "N/A")
        r1c2.metric("Market Cap", f"${cap:,.0f}" if cap else "N/A")
        r2c1.metric("52w High / Low", f"${high} / ${low}")

        hist_5d = get_stock_data(ticker, "5d")
        if hist_5d is not None and len(hist_5d) >= 2:
            change = hist_5d["Close"].iloc[-1] - hist_5d["Close"].iloc[-2]
            pct = (change / hist_5d["Close"].iloc[-2]) * 100
            r2c2.metric("Daily Change", f"${change:.2f}", f"{pct:.2f}%")

        # SUMMARY
        summary = info.get("longBusinessSummary", "No company description available.")
        if summary.strip():
            with st.expander("📘 Company Info"):
                st.write(summary)

        st.markdown("---")

        # NEWS
        st.subheader(f"📰 {ticker} Recent News")
        news = get_company_news(ticker, finnhub_api) if finnhub_api else []
        if not finnhub_api:
            st.info("Enter Finnhub API key to enable news.")
        if news:
            for a in news[:5]:
                dt = datetime.datetime.fromtimestamp(a.get("datetime", 0))
                date = dt.strftime("%b %d, %Y")
                safe_markdown(
                    f"<div class='news-card'><a href='{a.get('url')}' target='_blank'><b>{a.get('headline')}</b></a><br><small>{a.get('source')} | {date}</small></div>"
                )

        safe_markdown("<hr style='opacity:0.3;'>")

    safe_markdown("<hr>")
    safe_markdown("Built with ❤️ — Wizard Q")

# RUN
if __name__ == "__main__":
    run_app()
